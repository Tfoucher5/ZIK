  ...
  try {
    ...
    while (offset < Math.min(total, 1000)) {
      const tRes = await fetchFn(
        `https://api.spotify.com/v1/playlists/${plId}/tracks?limit=100&offset=${offset}&fields=items%28track%28id%2Ctype%2Cname%2Cartists%28name%29%2Calbum%28images%29%2Cpreview_url`,
        { headers: { Authorization: `Bearer ${currentToken}` } }
      );
